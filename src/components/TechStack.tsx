...
const TechStack: React.FC = () => {
    ...
    return (
        <section id="tech" className="py-24 relative z-10 overflow-hidden border-y border-white/5 bg-void-900/60 backdrop-blur-md">
            ...
            <div className="flex overflow-hidden w-full relative">
                <div className="absolute inset-y-0 left-0 w-48 bg-gradient-to-r from-void-950 via-void-950/80 to-transparent z-10 pointer-events-none"></div>
                <div className="absolute inset-y-0 right-0 w-48 bg-gradient-to-l from-void-950 via-void-950/80 to-transparent z-10 pointer-events-none"></div>

                <div className="flex w-max animate-scrolling-logos space-x-24 items-center pl-10">
                    {doubledTechs.map((tech, index) => (
                        <div
                            key={index}
                            className="flex items-center space-x-4 grayscale opacity-40 hover:grayscale-0 hover:opacity-100 hover:text-raiden-400 transition-all duration-300 transform hover:scale-110 cursor-pointer"
                        >
